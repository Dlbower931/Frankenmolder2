...
Also, subscribes to ROS command topics (Setpoints, State Cmds) from the GUI
and sends the corresponding commands over the CAN bus to the ESP32.

Publishes raw CAN messages to /can_bus_raw_string for Foxglove debugging.
"""
...
class CANBridgeNode:
    def __init__(self):
        ...
        # Actual State Publishers (ESP32 reports its true state)
        ...
        # Temperature Error State Publishers (For sensor faults)
        # We can re-use the temp_pubs keys for simplicity
        ...
        try:
            if CAN_INTERFACE not in os.listdir('/sys/class/net/'):
                ...
                rospy.logfatal(f"Ensure 'network_mode: host' is in docker-compose.yml")
        ...
        # Heater State Commands (Using lambda to pass the zone_id)
    ...
    def ros_motor_rpm_callback(self, msg):
        ...
        rpm_val = msg.data
        # Format as "RPM10.0" to match what Pico/ESP32 expects
    ...
    def ros_heater_state_callback(self, msg, zone_id):
        ...
        # Send the zone_id and state as a single string, e.g. "zone1:HEATING"
        command_str = f"{zone_id}:{msg.data}"
        rospy.loginfo(f"ROS->CAN: Received {zone_id} state command: {msg.data}")
        self.send_can_string(CAN_ID_CMD_STATE, command_str)
    ...
    def run_listener(self):
        ...
        try:
            for message in self.bus: # This loop blocks until a message is received
                ...
                # --- Publish raw CAN message as a string for debugging ---
                try:
                ...
                # --- Handle Other Status Messages (Motor, etc.) ---
                # (Add elif blocks here when ESP32 starts sending them)
